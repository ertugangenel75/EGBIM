...
Toplam **82 manifest**, 8 blok. Gerçek dünya AEC ihtiyaçlarına göre tasarlanmıştır.
...
## 🏭 Blok 8 — Üretim / Otomasyon (12)

Model **yazma/üretim** manifestleri. Önceki bloklar analiz-odaklıydı; bu blok Excel/CSV verisinden veya mevcut elemanlardan **yeni eleman üretir**. Tümü `preview_gate` veya `atomic` transaction ile güvenli çalışır, `trace_key` ile idempotenttir (tekrar çalıştırınca çoğaltmaz).

> **`$INPUT` dialog:** Bu bloktaki manifestler aile adı, tip, seviye ve dosya yolu gibi projeye özgü değerleri sabit yazmaz; çalışma anında bir dialog açıp kullanıcıya sorar. Seviyeler Revit modelinden otomatik dropdown olarak gelir, dosya seçimi göz-at penceresiyle yapılır. Aynı değer (örn. "Hangi kat") manifest içinde birden çok yerde geçse bile **bir kez** sorulur. Manifest çalıştırılmadan önce bu pencere otomatik belirir.

> **Aile + tip tek dropdown:** Yapısal/mimari eleman üreten manifestlerde aile ve tip artık ayrı ayrı yazılmaz; modelde yüklü ailelerden tek bir açılır listeden seçilir (örn. *M_Kolon : 300x300*). Seçim arka planda otomatik olarak `family_name` ve `type_name` alanlarına bölünür.

| # | Dosya | Başlık | Adım | Açıklama |
|---|-------|--------|------|----------|
| 71 | `71_excelden_kolon_uretim.json` | Excel'den Kolon Üretimi | 9 | Excel/CSV koordinat tablosundan yapısal kolonları otomatik üretir. Tabloyu XYZ... |
| 72 | `72_excelden_kiris_uretim.json` | Excel'den Kiriş Üretimi | 10 | CSV başlangıç-bitiş koordinat tablosundan yapısal kirişleri üretir. Noktalarda... |
| 73 | `73_akslardan_grid_uretim.json` | Akslardan Grid Üretimi | 8 | CSV aks koordinat tablosundan Revit gridlerini (akslarını) otomatik üretir. Ba... |
| 74 | `74_odalardan_doseme_uretim.json` | Odalardan Döşeme Üretimi | 10 | Modeldeki odaların sınırlarından otomatik döşeme üretir. Her oda için kat plan... |
| 75 | `75_cizgilerden_duvar_uretim.json` | Excel'den Duvar Üretimi | 11 | CSV başlangıç-bitiş koordinat tablosundan mimari duvarları üretir. Çizgilerden... |
| 76 | `76_duvara_armatur_yerlesim.json` | Duvara Host Aile Yerleştirme | 8 | Seçili kattaki duvarlara host-based aile (priz, anahtar, armatür) otomatik yer... |
| 77 | `77_tavana_aydinlatma_yerlesim.json` | Tavana Aydınlatma Yerleştirme | 11 | Seçili kattaki tavanlara face-based aydınlatma armatürü otomatik yerleştirir. ... |
| 78 | `78_aile_kutuphane_tip_uret.json` | Aile Kütüphane Kurulumu ve Tip Üretimi | 10 | Bir .rfa ailesini projeye yükler, içine parametrik boyut ekler ve standart tip... |
| 79 | `79_otomatik_pafta_gorunum.json` | Otomatik Pafta ve 3D Görünüm Üretimi | 7 | Yeni bir pafta oluşturur, modelden 3D görünüm üretir ve görünümü paftaya yerle... |
| 80 | `80_uretilenleri_temizle.json` | Üretilen Elemanları Temizle | 4 | Daha önce trace_key ile üretilmiş elemanları bulur ve güvenli şekilde siler. c... |
| 81 | `81_eleman_duzenleme_otomasyon.json` | Eleman Düzenleme ve Çoğaltma | 8 | Seçili elemanları taşır veya aynalayarak çoğaltır; ardından yeni oda ve nokta-... |
| 82 | `82_adaptif_cephe_panel.json` | Adaptif Cephe Paneli Yerleştirme | 9 | CSV nokta tablosundan adaptif cephe panellerini üretir. Her panel için gereken... |
